      ...
      await env.DB.prepare(
        `INSERT INTO characters
           (char_id, name, realm, display, show_realm, class, faction, level,
            server, alive, owner_install, first_seen, updated)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
        .bind(
          ...
          blob.server || null, blob.alive === false ? 0 : 1, installId, t, t
        )
        .run();
    } else {
      await env.DB.prepare(
        `UPDATE characters SET display = ?, show_realm = ?, class = ?, faction = ?,
                               level = ?, server = ?, alive = ?, updated = ?
         WHERE char_id = ? AND owner_install = ?`
      )
        .bind(
          blob.display || name, blob.showRealm === false ? 0 : 1,
          blob.class || null, blob.faction || null, blob.level || null,
          blob.server || null, blob.alive === false ? 0 : 1, t,
          charId, installId
        )
        .run();
    }
...
async function handleLeaderboard(url, env) {
  ...
  const rs = await env.DB.prepare(
    `SELECT p.char_id, p.metric, p.pct, p.band,
            c.display, c.realm, c.show_realm, c.class, c.faction, c.level, c.server, c.alive,
      ...
      ORDER BY ${
        // The levelling board is a RACE ladder: a character at 80 outranks a
        // character at 6 no matter how quick the 6's levels were. Speed
        // breaks ties within a level; the colour band stays the lvl/hr
        // percentile. A player levelled 1-80 in three days and sat below a
        // level-6 twink -- that is what this ordering fixes.
        board === 'levelling' ? 'c.level DESC, p.metric DESC' : 'p.metric DESC'
      }
      LIMIT ?`
  )
